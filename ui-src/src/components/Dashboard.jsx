...
import { Play, Stop, Key, Spinner, CheckCircle, XCircle, ArrowsClockwise, WarningOctagon, Clock, ChatCircle, Newspaper, Database, WechatLogo, Brain, Robot, Cube, TestTube, PaperPlaneTilt, Lightning } from '@phosphor-icons/react'
...
function MessageInjectPanel() {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ ...spring, delay: 0.24, duration: 0.5 }}
      className="bg-bg-card border border-border-main rounded-2xl overflow-hidden"
    >
      ...
      <div className="px-6 pb-5 space-y-3">
        ...
        {/* Row 2: Message input */}
        <div>
          <label className="text-[11px] text-text-muted font-medium mb-1 block">消息内容</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg bg-bg-raised border border-border-main text-[13px] text-text-main placeholder:text-text-muted/30 focus:outline-none focus:border-brand-green/40 transition-colors"
            />
            <motion.button
              whileTap={{ scale: 0.96 }}
              onClick={handleSend}
              disabled={sending || !content.trim()}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-[12px] font-semibold bg-brand-green text-white hover:opacity-90 transition-all disabled:opacity-40 cursor-pointer"
            >
            ...
            >
              {result.ok ? (
                ...
                <div className="flex items-start gap-2 p-3 rounded-lg bg-status-error/[0.04] border border-status-error/15">
                  <XCircle size={14} weight="fill" className="text-status-error mt-0.5 flex-shrink-0" />
